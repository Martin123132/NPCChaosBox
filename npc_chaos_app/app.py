...
from .engine import generate_npc
from . import storage


ROOT = storage.repo_root()
STATIC_DIR = ROOT / "npc_chaos_app" / "static"
TEMPLATE_DIR = ROOT / "npc_chaos_app" / "templates"
...
class NPCChaosHandler(BaseHTTPRequestHandler):
    server_version = f"NPCChaosBox/{__version__}"

    def log_message(self, fmt: str, *args: object) -> None:
        sys.stdout.write("[npc-chaos] " + fmt % args + "\n")
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        try:
            if path == "/":
                ...
                self._json({"ok": True, "state": storage.load_state(), "doctor": storage.doctor(), "version": __version__})
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if path == "/api/state":
                ...
                self._json({"ok": True, "state": state, "doctor": storage.doctor()})
            elif path == "/api/generate":
                scene = generate_npc(storage.load_state(), payload)
                self._json({"ok": True, "scene": scene})
            elif path == "/api/favourites":
                scene = payload.get("scene") or {}
                if not isinstance(scene, dict) or not scene.get("npc"):
                    self._json({"ok": False, "error": "No NPC supplied"}, HTTPStatus.BAD_REQUEST)
                    return
                ...
                if not isinstance(scene, dict) or not scene.get("npc"):
                    self._json({"ok": False, "error": "No NPC supplied"}, HTTPStatus.BAD_REQUEST)
                    return
                result = storage.export_npc(scene, str(payload.get("format") or "txt"))
...
def run(host: str = "127.0.0.1", port: int = 0, open_browser: bool = True) -> None:
    server = ThreadingHTTPServer((host, port), NPCChaosHandler)
    actual_port = server.server_address[1]
    url = f"http://{host}:{actual_port}"
    print(f"NPC Chaos Box is running at {url}", flush=True)
    ...
    try:
        ...
        print("\nNPC Chaos Box stopped.")
    finally:
        server.server_close()


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Run the local NPC Chaos Box app.")
